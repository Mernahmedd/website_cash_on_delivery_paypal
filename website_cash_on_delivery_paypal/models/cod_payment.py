...
class CODAcquirer(models.Model):
	...
	code = fields.Selection(selection_add=[('cod_paypal', 'Cash Paypal')], ondelete={'cod_paypal': 'set default'})

	def _get_providers(self):
		providers = super(CODAcquirer, self)._get_providers()
		providers.append(['cod_paypal', _('Cash on Delivery')])
		return providers

	def cod_paypal_get_form_action_url(self):
		return '/cod_paypal/payment/feedback'

	def cod_paypal_compute_fees(self, amount, currency_id, country_id):
		""" Compute paypal fees.
...
class CODPaymentTransaction(models.Model):
	...
	@api.model
	def cod_paypal_form_get_tx_from_data(self, data):
		...
		for order in tx.sale_order_ids:
			order.cod_paypal_available = True
	...
	def _get_specific_rendering_values(self, processing_values):
		...
		if self.provider_id.code != 'cod_paypal':
			return res

		else :
			rendering_values = {
				'reference': self.reference,
				'tx_url': self.provider_id.cod_paypal_get_form_action_url(),
